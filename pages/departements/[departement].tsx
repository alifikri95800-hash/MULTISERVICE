...
import { Departement, getAllDepartementSlugs, getDepartementBySlug } from '../../src/data/departements';
...
export default function DepartementPage({ departement }: DepartementPageProps) {
  ...
  const structuredData = {
    ...
    "@type": "LocalBusiness",
    "name": `FKR MULTI-SERVICES - ${departement.name}`,
    "description": departement.description,
    "url": `https://example.com/departements/${departement.slug}`,
  ...
  return (
    <>
      <Head>
        <title>{departement.title}</title>
        <meta name="description" content={departement.description} />
        <link rel="canonical" href={`https://example.com/departements/${departement.slug}`} />
        <meta property="og:title" content={departement.title} />
        <meta property="og:description" content={departement.description} />
        <meta property="og:type" content="website" />
        <meta property="og:url" content={`https://example.com/departements/${departement.slug}`} />
        <script
      ...
      <div className="min-h-screen bg-white">
        <Header 
          currentPage={`departement-${departement.slug}`}
          onNavigate={navigate}
                ...
                <div className="grid lg:grid-cols-2 gap-12 items-center">
                  <div className="text-center lg:text-left">
                    ...
                    <p className="text-xl text-gray-600 mb-8">
                      {departement.description}. Intervention rapide 24h/7j par des techniciens certifiés.
                    </p>
                  ...
                  <div className="relative rounded-2xl overflow-hidden shadow-2xl">
                    <img
                      ...
                      alt={`Services de plomberie, chauffage et électricité - ${departement.name}`}
                    ...
                    <div className="absolute bottom-6 left-6 bg-white/95 backdrop-blur-sm rounded-xl p-4 shadow-lg">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          <p className="text-blue-600 text-sm">{departement.name}</p>
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="prose prose-lg max-w-none">
                  <p className="text-gray-700 mb-8 text-lg leading-relaxed">
                    {departement.content.intro}
                  </p>

                  <p className="text-gray-700 mb-12 text-lg leading-relaxed">
                    Nos équipes de <strong>plombiers dépannage {departement.name}</strong> sont disponibles 24h/7j pour intervenir 
                    en urgence dans tout le département {departement.number}. Chaque déplacement est précédé d'un devis gratuit et transparent, 
                    et nous assurons une arrivée sur place rapidement.
                  </p>

                  <h2 className="text-3xl font-bold text-gray-900 mb-8 flex items-center">
                    🚨 Dépannage plomberie urgent dans le {departement.name}
                  </h2>

                  <p className="text-gray-700 mb-6">
                    Nous couvrons toutes les urgences plomberie dans le {departement.name} :
                  </p>

                  <div className="bg-blue-50 rounded-2xl p-8 mb-12">
                    <ul className="space-y-4">
                      {departement.content.services.map((service, index) => (
                  ...
                  <div className="grid md:grid-cols-2 gap-8 mb-12">
                    <div className="bg-green-50 rounded-xl p-6">
                      ...
                      <ul className="space-y-2 text-green-800">
                        ...
                        <li>• Service de proximité couvrant {departement.cities.join(', ')}</li>
                      </ul>
                    </div>
                    <div className="bg-blue-50 rounded-xl p-6">
                      <h3 className="font-bold text-blue-900 mb-4">Villes couvertes</h3>
                      <ul className="space-y-2 text-blue-800">
                        {departement.cities.map((city, index) => (
                  ...
                  <div className="bg-yellow-50 border-l-4 border-yellow-400 p-6 mb-12">
                    <p className="text-yellow-800 font-semibold">
                      👉 Dans le {departement.name}, nos plombiers assurent une intervention rapide, où que vous soyez.
                    </p>
                  </div>

                  <h2 className="text-3xl font-bold text-gray-900 mb-8 flex items-center">
                    💶 Tarifs et devis dépannage {departement.name}
                  </h2>
                  ...
                  <h2 className="text-3xl font-bold text-gray-900 mb-8 flex items-center">
                    ✅ Pourquoi choisir notre service dans le {departement.name} ?
                  </h2>

                  <div className="grid md:grid-cols-2 gap-8 mb-12">
                    <div className="space-y-4">
                      {departement.content.advantages.slice(0, 3).map((advantage, index) => (
                    ...
                    <div className="space-y-4">
                      {departement.content.advantages.slice(3).map((advantage, index) => (
                        <div key={index} className="flex items-start space-x-3">
                          <CheckCircle className="h-5 w-5 text-blue-600 mt-1 flex-shrink-0" />
                          <span className="text-gray-700">{advantage}</span>
                        </div>
                  ...
                  <h2 className="text-3xl font-bold text-gray-900 mb-8 flex items-center">
                    ❓ FAQ – Dépannage plomberie {departement.name}
                  </h2>

                  <div className="space-y-6 mb-12">
                    {departement.content.faq.map((faq, index) => (
                  ...
                  <div className="bg-blue-600 text-white rounded-2xl p-8 text-center">
                    <h2 className="text-2xl font-bold mb-4">
                      📞 Appelez dès maintenant un plombier dépannage {departement.name}
                    </h2>
                  ...
                  <div className="mt-16 pt-8 border-t border-gray-200">
                    <h3 className="text-lg font-semibold text-gray-900 mb-4">Nos services dans le {departement.name}</h3>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-sm">
                      <a href="/plomberie" className="text-blue-600 hover:text-blue-800 transition-colors">Plomberie {departement.number}</a>
                      <a href="/chauffage" className="text-blue-600 hover:text-blue-800 transition-colors">Chauffage {departement.number}</a>
                      <a href="/electricite" className="text-blue-600 hover:text-blue-800 transition-colors">Électricité {departement.number}</a>
...
export const getStaticPaths: GetStaticPaths = async () => {
  const slugs = getAllDepartementSlugs();
  const paths = slugs.map((slug) => ({
    params: { departement: slug }
  }));
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  const departement = getDepartementBySlug(params?.departement as string);
